...
const validarRegister = () => {
    return new Promise((resolve, reject) => {
        if (email.value.length < 10 || email.value.length > 40 || email.value.indexOf("@") == -1 || email.value.indexOf(".") == -1) reject("El correo electronico es invalidó");
        else if (nombre.value.length < 10 || nombre.value.length > 60) reject("El nombre es invalidó");
        else if (usuario.value.length < 5 || usuario.value.length > 15) reject("El usuario es invalidó");
        else if (password.value.length < 10 || password.value.length > 30 || verificar()) reject("La contraseña es invalida");
        resolve(true);
    })
}
